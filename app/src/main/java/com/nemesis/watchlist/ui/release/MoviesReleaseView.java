...
import com.nemesis.watchlist.data.repository.ReleaseRepositoryImpl;
...
public class MoviesReleaseView extends ViewModel {
    MutableLiveData<ResponseError> responError = new MutableLiveData<>();
    MutableLiveData<List<ResultsMovies>> releaseLiveData = new MutableLiveData<>();
    private ReleaseRepositoryImpl releaseRepoImpl;

    public MoviesReleaseView(){
        releaseRepoImpl = new ReleaseRepositoryImpl();
    }

    void getRelease(String tanggal) {
        releaseRepoImpl.getReleaseMovies(tanggal)
                ...
                .subscribe(new Observer<Response<ResponseMovies>>() {
                    ...
                    public void onNext(Response<ResponseMovies> response) {
                        if (response.body() != null && response.isSuccessful()){
                            releaseLiveData.postValue(response.body().getResults());
                            ...
                            if (resBody != null){
                                ...
                                if (jsonObject != null) {
                                    ...
                                    responError.postValue(resError);
                                }
